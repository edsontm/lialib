...
class SGDHD(Optimizer):
    ...
    def step(self, closure=None):
        ...
        group['lr'] += self.defaults['hypergrad_lr'] * h
...
class AdamHD(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, hypergrad_lr=1e-6):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if 'betas'  not in group.keys():
                    group['betas'] = self.defaults['betas']
                if 'eps' not in group.keys():
                    group['eps'] = self.defaults['eps']
                if 'hypergrad_lr' not in group.keys():
                    group['hypergrad_lr'] = self.defaults['hypergrad_lr']
